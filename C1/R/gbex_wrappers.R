...
gbex_fit <- function(X, y, intermediate_quantiles, interm_lvl, intermediate_q_feature = FALSE, scale_features = FALSE, ...) {
  ...
  gbex_obj <- list(
    fit_gbex = fit_gbex, intermediate_q_feature = intermediate_q_feature,
...
gbex_predict <- function(fitted_gbex, X_test, to_predict, intermediate_quantiles, interm_lvl = fitted_gbex$interm_lvl) {
  if (interm_lvl != fitted_gbex$interm_lvl) {
    stop("gbex intermediate quantiles interm_lvl does not match in train and predict.")
  }

  X_feats <- X_test
  if (fitted_gbex$intermediate_q_feature) {
    if (is.null(intermediate_quantiles)) {
      stop("intermediate_quantiles needed in gbex_predict for intermediate_q_feature.")
    }
...
gbex_CV <- function(X, y, intermediate_quantiles, interm_lvl, intermediate_q_feature = FALSE, scale_features = FALSE,
  ...
  if (is.null(min_leaf_size)) {
    min_leaf_size <- rep(max(10, length(Y_excesses) / 100), 2)
  }
  ...
  results_grid_fit <- foreach(depth = grid_depth, .errorhandling = err_handling, .combine = rbind) %fun% {
    ...
    CV_lambda_ratio <- gbex::CV_gbex(
      y = Y_excesses, X = data.frame(X_feats_excesses), num_folds = num_folds, Bmax = Bmax, par_name = "lambda_ratio", par_grid = grid_lambda_ratio,
  ...
  best_params <- results_tibble %>%
    slice_min(dev_best) %>%
    slice_min(depth_shape) %>%
    slice_min(depth_scale) %>%
    slice_max(lrr_opt) %>%
    as.list()
  ...
  return(list(
    best_params = best_params_renamed, results_tibble = results_tibble,
...
gbex_excess_probability <- function(fitted_gbex, val, X_test, intermediate_quantiles, interm_lvl = fitted_gbex$interm_lvl,
  ...
  Probs <- GPD_excess_probability(val,
    sigma = pred_params$s, xi = pred_params$g, interm_threshold = intermediate_quantiles,
