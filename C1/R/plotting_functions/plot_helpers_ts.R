plot_predictions_ts <- function(pred_q, pred_q_other, true_q, y = NaN, prob_lvl_predicted, logscale = FALSE, name_other = "GRF") {
  ...
  pred_plot <- preds_df %>%
    tidyr::gather(key = "Method", value = "Prediction", .data[[name_other]], Truth, EQRN, factor_key = TRUE) %>%
    ggplot(aes(x = t, y = Prediction, group = Method, color = Method)) +
    geom_line() +
    geom_point(data = data.frame(t = seq_along(true_q), y = y, Method = factor("observations")), aes(x = t, y = y, group = Method, color = Method)) +
  ...
  if (logscale) {
    pred_plot <- pred_plot + scale_y_log10()
  }
  return(pred_plot)
}
...
plot_predictions_diff_ts <- function(pred_q, pred_q_other, true_q, prob_lvl_predicted, logscale = FALSE, name_other = "GRF") {
  ...
  preds_df <- preds_df %>% rename_with(~ str_replace(., "Other", name_other))
  pred_plot <- preds_df %>%
    tidyr::gather(key = "Method", value = "Prediction", .data[[name_other]], EQRN, factor_key = TRUE) %>%
    ggplot(aes(x = t, y = Prediction, group = Method, color = Method)) +
    geom_line() +
    scale_color_manual(values = alpha(c("#69b3a2", rgb(57 / 255, 106 / 255, 177 / 255)), c(0.5, 1))) +
    labs(title = paste0("Predictions (q=", prob_lvl_predicted, ")"), x = "time", y = "Predictions", color = NULL)
  if (logscale) {
    pred_plot <- pred_plot + scale_y_log10()
  }
...
plot_rmse_quantile_ts <- function(fit_eqrn, fit_grf, fit_gbex, fit_egam, Y_train, X_test, y_test, prob_lvls_predict, pred_interm,
  ...
  pred_semicond <- predict_GPD_semiconditional(
    Y = Y_train[(seq_len + 1):length(Y_train)], interm_lvl = interm_lvl,
  ...
  pred_exgam <- predict_gpd_gam(fit_egam, lagged_test,
...
plot_exceedence_proba_ts <- function(exceedence_proba, exceedence_level, Date_index = NULL,
                                     ...
                                     event_dates = NULL, p_lab = if (proba_ratio) {
                                       "Probability ratio"
                                     } else {
  ...
  if (proba_ratio & log_scaling) {
    stop("Only one of 'proba_ratio' or 'log_scaling' is allowed in 'plot_exceedence_proba_ts'.")
  }
  ep <- as.numeric(exceedence_proba)
  exceedence_thresh <- if (type_probas == "probability") {
    1 - exceedence_level
  } else {
    exceedence_level
  }
  ...
  if (!is.null(Date_index)) {
    ...
    ep_plot <- df %>% ggplot(aes(x = x_ind, y = Proba)) +
      geom_line(color = color_met) +
      labs(y = p_lab, x = "Date") +
  ...
  ep_plot <- ep_plot + if (log_scaling) {
    scale_y_continuous(trans = trans_logp1, labels = logp1_ticks, expand = c(0.02, 0))
  } else {
    scale_y_continuous(expand = c(0.02, 0))
  }
  if (!is.null(event_dates)) {
    ep_plot <- ep_plot + geom_vline(xintercept = event_dates, linetype = "dashed")
  }
  return(ep_plot)
}
...
plot_exceedence_proba_box <- function(exceedence_proba, exceedence_level, type_probas = c("probability", "years"),
  ...
  ep_plot <- data.frame(x = "EQRN", Proba = ep) %>% ggplot(aes(x = x, y = Proba)) +
    geom_hline(yintercept = as.numeric(exceedence_thresh), linetype = 4, colour = my_palette$red) +
...
plot_pred_vs_return_lvl_ts <- function(preds, return_level, PoT_quantile, empirical_quantile,
  ...
  ablines <- data.frame(
    intercepts = c(return_level, PoT_quantile, empirical_quantile), slopes = c(0, 0, 0),
    cols = c("Block-Maxima", "PoT", "Empirical")
  )
  lims <- c(
    min(preds, return_level, PoT_quantile, empirical_quantile),
  ...
  if (!is.null(Date_index)) {
    df$Date <- Date_index
    df$x_ind <- Date_index
    ep_plot <- df %>% ggplot(aes(x = x_ind, y = preds, group = Method, color = Method, linetype = Method)) +
      geom_line() +
  ...
  ep_plot <- ep_plot + scale_color_manual(values = c(
    "Prediction" = color_met, "Block-Maxima" = my_palette$red,
  ...
  if (!is.null(y_obs)) {
    ep_plot <- ep_plot + geom_point(aes(y = y_ex), show.legend = FALSE, color = "darkslategrey", alpha = 0.8)
  }
  return(ep_plot)
}
...
plot_pred_quant_risk_ts <- function(preds, exceedence_proba, exceedence_level, return_level, PoT_quantile, empirical_quantile,
                                    ...
                                    event_dates = NULL, legend.position = "bottom", p_lab = if (proba_ratio) {
                                      "'Probability ratio'"
                                    } else {
                                      "'Probability'"
                                    },
  ...
  ablines <- data.frame(
    intercepts = c(return_level, PoT_quantile, empirical_quantile, as.numeric(exceedence_thresh)),
  ...
  if (!is.null(Date_index)) {
    df$Date <- Date_index
    df$x_ind <- Date_index
    ep_plot <- df %>% ggplot(aes(x = x_ind, y = Prediction, group = Method, color = Method, linetype = Method)) +
      geom_line() +
  ...
  ep_plot <- ep_plot +
    scale_color_manual(values = c(
      "Prediction" = color_met, "Block-Maxima" = my_palette$red, "B-M threshold" = my_palette$red,
      "PoT" = my_palette_methods[["Uncond"]], "Empirical" = my_palette$light_blue
    )) +
  ...
  ep_plot <- ep_plot +
    if (log_scaling) {
      stop("'log_scaling' not yet possible in 'plot_pred_quant_risk_ts'.")
    } else {
      scale_y_continuous(expand = c(0.02, 0))
    }
  if (!is.null(event_dates)) {
...
plot_features_for_pred_ts <- function(Y, X, Z, predictions, return_level, event_ind, Date_index = NULL, int_index = seq_along(Y), seq_len = 10,
  ...
  ablines <- data.frame(
    intercepts = c(return_level), slopes = c(0), cols = c("Block-Maxima"),
  ...
  if (!is.null(Date_index)) {
    ...
    df <- data.frame(
      id = c(int_index, NA), Date = c(Date_index, event_ind),
    ...
    sc_plot <- df %>% ggplot(aes(x = Date, y = Val, color = Features)) +
      ...
      geom_point(aes(shape = Features, size = Features)) +
      labs(y = NULL, x = "Date", color = NULL, linetype = NULL, shape = NULL) +
      scale_x_date(
        breaks = break_f, minor_breaks = mbreak_f, date_labels = "%Y %b %d",
      ...
      facet_grid(Station ~ ., scales = "free_y", switch = "y", labeller = label_parsed) +
      theme(panel.grid.major.x = element_line(size = 0.5), axis.ticks.x = element_line(size = 0.5), legend.position = legend.position)
  } else {
    pred <- predictions[which(int_index == event_ind)]
    df <- data.frame(
      id = c(int_index, event_ind),
      Y = c(Y, pred), X = c(X, NA), Z = c(Z, NA), Features = c(rep(NA, length(Y)), "Pred. Quantile")
    )
...
plot_series_comparison2 <- function(Y, X, Date_index = NULL, int_index = seq_along(Y),
  ...
  if (!is.null(Date_index)) {
    ...
    sc_plot <- df %>% ggplot(aes(x = Date, y = Val)) +
      geom_line() +
      labs(y = NULL, x = "Date") +
      scale_x_date(
        breaks = break_f, minor_breaks = mbreak_f, date_labels = "%Y %b",
    ...
    sc_plot <- df %>% ggplot(aes(x = id, y = Val)) +
      geom_line() +
      labs(y = NULL, x = NULL) +
      scale_x_continuous(expand = c(0.01, 0)) +
      scale_y_continuous(expand = c(0.02, 0)) +
      facet_wrap(~Variable, scales = "free_y", ncol = 1, strip.position = "left", labeller = label_parsed)
  }
  if (!is.null(event_dates)) {
...
plot_series_comparison3 <- function(Y, X, Z, Date_index = NULL, int_index = seq_along(Y),
  ...
  if (!is.null(Date_index)) {
    ...
    sc_plot <- df %>% ggplot(aes(x = id, y = Val)) +
      ...
      scale_x_continuous(expand = c(0.01, 0)) +
      scale_y_continuous(expand = c(0.02, 0)) +
      facet_wrap(~Station, scales = "free_y", ncol = 1, strip.position = "left", labeller = label_parsed)
  }
  if (!is.null(event_dates)) {
    sc_plot <- sc_plot + geom_vline(xintercept = event_dates, linetype = "dashed")
  }
...
plot_series_comparison <- function(df, col1, col2, datecol = "Date") {
  ps1 <- data.frame(df) %>% ggplot(aes_string(x = datecol, y = col1)) +
    geom_line() +
    labs(y = col1, x = NULL) +
  ...
  ps2 <- data.frame(df) %>% ggplot(aes_string(x = datecol, y = col2)) +
    geom_line() +
    labs(y = col2, x = "Date") +
    scale_x_date(date_breaks = "1 month", date_minor_breaks = "1 day", date_labels = "%Y %b", expand = c(0.01, 0)) +
    scale_y_continuous(expand = c(0.02, 0)) +
...
plot_series_comparison_sim <- function(Y, X) {
  ps1 <- data.frame(id = seq_along(Y), Y = Y, X = X) %>% ggplot(aes(x = id, y = Y)) +
    geom_line() +
    labs(y = "Y", x = NULL) +
    theme(axis.ticks.x = element_blank(), axis.text.x = element_blank())
  ps2 <- data.frame(id = seq_along(Y), Y = Y, X = X) %>% ggplot(aes(x = id, y = X)) +
    geom_line() +
    labs(y = "X", x = NULL)
  sc_plot <- ggpubr::ggarrange(ps1, ps2, labels = NULL, ncol = 1, nrow = 2, align = "v")
  return(sc_plot)
...
plot_error_quantile_ts_old <- function(fit_eqrn, fit_grf, fit_gbex, fit_egam, Y_train, X_test, y_test, prob_lvls_predict, pred_interm,
  ...
  if (factorize) {
    df_rmse <- data.frame(
      Quantile = as_factor(roundm(prob_lvls_predict, 4)), Uncond = RMSEs_unc, Semi_cond = RMSEs_semicond,
      GRF = RMSEs_grf, gpd_GAM = RMSEs_exgam, gbex = RMSEs_gbex, EQRN = RMSEs_eqrnn
    )
    df_mae <- data.frame(
      Quantile = as_factor(roundm(prob_lvls_predict, 4)), Uncond = MAEs_unc, Semi_cond = MAEs_semicond,
      GRF = MAEs_grf, gpd_GAM = MAEs_exgam, gbex = MAEs_gbex, EQRN = MAEs_eqrnn
    )
  } else {
    df_rmse <- data.frame(
      Quantile = prob_lvls_predict, Uncond = RMSEs_unc, Semi_cond = RMSEs_semicond,
      GRF = RMSEs_grf, gpd_GAM = RMSEs_exgam, gbex = RMSEs_gbex, EQRN = RMSEs_eqrnn
    )
    df_mae <- data.frame(
      Quantile = prob_lvls_predict, Uncond = MAEs_unc, Semi_cond = MAEs_semicond,
      GRF = MAEs_grf, gpd_GAM = MAEs_exgam, gbex = MAEs_gbex, EQRN = MAEs_eqrnn
    )
  }
  ...
  mse_plot <- df_rmse %>%
    tidyr::gather(key = "Model", value = "Error", Uncond, Semi_cond, GRF, gpd_GAM, gbex, EQRN, factor_key = TRUE) %>%
    ggplot(aes(x = Quantile, y = Error, group = Model, color = Model, linetype = Model)) +
    geom_line(size = 1) +
    scale_linetype_manual(values = linetypes) +
    scale_color_manual(values = color_scale) +
