training_plot_eqrn <- function(fit_eqrn, uncond_losses_interm, title = NULL, y_lab = "GPD likelihood loss", show_legend = TRUE) {
  ablines <- data.frame(
    intercepts = c(uncond_losses_interm$train_loss, uncond_losses_interm$valid_loss), slopes = c(0, 0),
  ...
  train_plot <- data.frame(Epoch = seq_along(fit_eqrn$train_loss), Train = fit_eqrn$train_loss, Validation = fit_eqrn$valid_loss) %>%
    ...
    geom_line(size = 1) +
    scale_color_manual(values = c(my_palette$red, my_palette$blue, my_palette$red, my_palette$blue), guide = guide_legend(reverse = TRUE)) +
...
validation_plot_eqrn <- function(fit_eqrn, uncond_losses_interm, title = NULL, y_lab = "GPD likelihood loss", show_legend = FALSE) {
  ablines <- data.frame(
    intercepts = c(uncond_losses_interm$valid_loss), slopes = c(0),
  ...
  train_plot <- data.frame(Epoch = seq_along(fit_eqrn$valid_loss), Validation = fit_eqrn$valid_loss) %>%
    ...
    ggplot(aes(x = Epoch, y = Value, group = Type, color = Type, linetype = Type)) +
    geom_line(size = 1) +
    scale_color_manual(values = c(my_palette_methods[["Semi-cond"]], my_palette_methods[["EQRN"]]), guide = guide_legend(reverse = TRUE)) +
    scale_linetype_manual(values = c("dashed", "solid"), guide = guide_legend(reverse = TRUE)) +
    geom_abline(data = ablines, aes(intercept = intercepts, slope = slopes, col = cols, linetype = cols), size = 1, show.legend = FALSE) +
    scale_y_continuous(expand = c(0.02, 0)) +
    scale_x_continuous(expand = c(0.02, 0)) +
    expand_limits(y = lims)
  labs(title = title, x = "Epoch", y = y_lab, color = NULL, linetype = NULL)
  if (show_legend) {
    train_plot <- train_plot + theme(
      legend.position = c(.99, .99),
...
plot_predictions_1D <- function(pred_eqrnn_test, pred_grf_test, pred_true, X_test, prob_lvl_predicted) {
  pred_plots <- list()
  for (j in 1:4) {
  ...
  pred_plot <- ggpubr::ggarrange(
    plotlist = pred_plots, labels = "AUTO", ncol = min(2, length(pred_plots)), nrow = 2,
    common.legend = TRUE, legend = "bottom", vjust = 1.5, align = "hv"
  ) %>%
    ggpubr::annotate_figure(
      top = text_grob(paste0("Predictions (q=", prob_lvl_predicted, ")"), face = "bold", size = 14),
      left = text_grob("Predictions", rot = 90)
    )
  return(pred_plot)
}

residuals_boxplot_eqrn <- function(pred_eqrnn_test, pred_grf_test, pred_true, prob_lvl_predicted) {
  res_box <- data.frame(GRF = c(pred_grf_test - pred_true), EQRNN = c(pred_eqrnn_test - pred_true)) %>%
    tidyr::gather(key = "Method", value = "Value", factor_key = TRUE) %>%
    ggplot(aes(x = Method, y = Value, fill = Method)) +
    geom_abline(intercept = 0, slope = 0, col = "white", size = 1) +
    geom_violin(width = 1, color = "grey", alpha = 0.5) +
    stat_boxplot(geom = "errorbar", width = 0.5) +
    geom_boxplot(width = 0.75, color = "black", alpha = 0.8) +
    coord_flip() +
  ...
  resid_boxes <- ggpubr::ggarrange(res_box, abs_box, sq_box, labels = NULL, ncol = 1, nrow = 3) %>%
    ggpubr::annotate_figure(top = text_grob(paste0("Quantile Residuals (q=", prob_lvl_predicted, ")"), face = "bold", size = 14))
  return(resid_boxes)
}

residuals_boxplot_eqrn2 <- function(pred_eqrnn_test, pred_grf_test, pred_gbex_test, pred_true, prob_lvl_predicted) {
  res_box <- data.frame(
    GRF = c(pred_grf_test - pred_true), gbex = c(pred_gbex_test - pred_true),
    ...
    theme(legend.position = "none", panel.grid.major.y = element_blank())
  abs_box <- data.frame(
    GRF = abs(c(pred_grf_test - pred_true)), gbex = abs(c(pred_gbex_test - pred_true)),
    ...
    theme(legend.position = "none", panel.grid.major.y = element_blank())
  sq_box <- data.frame(
    GRF = (c(pred_grf_test - pred_true))^2, gbex = (c(pred_gbex_test - pred_true))^2,
...
plot_predictions_vs_truth <- function(pred_eqrnn, pred_other, pred_semicond, pred_true, prob_lvls_predicted, name_other = "GRF",
                                      legend.position = "bottom") {
  nb_prob_lvls_predict <- length(prob_lvls_predicted)
  pred_vs_true_plots <- list()
  col_other <- if (is.na(my_palette_methods[name_other])) {
    my_palette$red
  } else {
    my_palette_methods[[name_other]]
  }
  for (i in 1:nb_prob_lvls_predict) {
    ...
    if (legend.position == "none") {
      ...
      pred_vs_true_plots[[i]] <- pred_vs_true_plots[[i]] +
        labs(title = paste0(prob_lvls_predicted[i] * 100, "% quantile"), x = NULL, y = NULL, color = NULL)
    }
  }
  pred_vs_true_plot <- ggpubr::ggarrange(
    plotlist = pred_vs_true_plots, labels = NULL, ncol = min(3, length(pred_vs_true_plots)),
...
plot_predictions_vs_truth_solo <- function(pred_eqrnn, pred_semicond, pred_true, prob_lvls_predicted,
                                           legend.position = "bottom") {
  nb_prob_lvls_predict <- length(prob_lvls_predicted)
  pred_vs_true_plots <- list()
  for (i in 1:nb_prob_lvls_predict) {
...
plot_predictions_vs_truth_comp <- function(pred_eqrnn1, pred_eqrnn2, pred_other, pred_true,
  ...
  preds_df <- data.frame(
    Truth = c(pred_true, pred_true), Other = c(pred_other, pred_other), EQRNN = c(pred_eqrnn1, pred_eqrnn2),
    EQRNN_mod = factor(c(rep(names_EQRN[1], n), rep(names_EQRN[2], n)), levels = names_EQRN)
  )
  lims <- c(
    min(pred_eqrnn1, pred_eqrnn2, pred_other, pred_true, na.rm = TRUE),
...
plot_predictions_vs_competitor <- function(pred_eqrnn, pred_comp, prob_lvls_predicted, name_comp = "Competitor", xaxis = c("EQRN", "competitor")) {
  #
  ...
  for (i in 1:nb_prob_lvls_predict) {
    ...
    pred_vs_comp_plots[[i]] <- tmp_plt +
      ...
      labs(title = paste0("Predicted ", prob_lvls_predicted[i] * 100, "% quantile"), x = NULL, y = NULL, color = NULL)
  }
  tmp_plt <- ggpubr::ggarrange(
    plotlist = pred_vs_comp_plots, labels = "AUTO", ncol = min(3, length(pred_vs_comp_plots)),
    common.legend = TRUE, legend = "bottom", vjust = 1.5, align = "hv"
  )
  if (xaxis == "competitor") {
...
plot_rmse_quantile <- function(fit_eqrn, fit_grf, fit_gbex, fit_egam, Y_train, X_test, prob_lvls_predict, intermediate_method = c("grf", "oracle"),
  ...
  pred_unc <- predict_unconditional_quantiles(interm_lvl = interm_lvl, quantiles = prob_lvls_predict, Y = Y_train, ntest = ntest)
  # SEMI-CONDITIONAL predicted quantiles
  pred_semicond <- predict_GPD_semiconditional(
    Y = Y_train, interm_lvl = interm_lvl, thresh_quantiles = interm_quant_train,
  ...
  pred_exgam <- predict_gpd_gam(fit_egam, X_test,
    to_predict = prob_lvls_predict,
    intermediate_quantiles = pred_interm, interm_lvl = interm_lvl
  )

  # Final EQRN predictions on X_test
  pred_eqrnn <- EQRN_predict(fit_eqrn, X_test, prob_lvls_predict, pred_interm, interm_lvl)
  ...
  Q_lvls <- if (factorize) {
    as_factor(roundm(prob_lvls_predict, 4))
  } else {
    prob_lvls_predict
  }
  ...
  df_rmse <- data.frame(
    Quantile = Q_lvls, Uncond = RMSEs_unc, Semi_cond = RMSEs_semicond,
  ...
  y_lab_rmse <- if (test_data == "halton") {
    "RISE"
  } else {
    "RMSE"
  }
  ...
  mse_plot <- df_rmse %>% ggplot(aes(x = Quantile, y = Error, group = Model, color = Model, linetype = Model)) +
    ...
    scale_y_continuous(expand = c(0.02, 0)) +
    labs(title = NULL, x = "Probability level", y = y_lab_rmse, color = NULL, linetype = NULL) +
    theme(legend.position = legend.position)

  mse_plot <- mse_plot + if (factorize) {
    scale_x_discrete(expand = c(0.02, 0))
  } else {
    scale_x_continuous(expand = c(0.02, 0))
  }
  if (crop_obs > 0) {
    mse_plot <- mse_plot + coord_cartesian(ylim = c(min(df_rmse$Error), sort(df_rmse$Error, decreasing = TRUE)[[crop_obs + 1]]))
  }
  ...
  if (!bias_variance) {
    ...
    Rbv_lims <- list(
      c(max(min(sort(df_Rbv$Error[df_Rbv$metric == Rbv_names[1]], decreasing = FALSE)[[crop_Rbv[1] + 1]], 0), -5.), 1.),
      (c(-1, 1) * sort(abs(df_Rbv$Error[df_Rbv$metric == Rbv_names[2]]), decreasing = TRUE)[[crop_Rbv[2] + 1]]),
      c(
        min(df_Rbv$Error[df_Rbv$metric == Rbv_names[3]]),
    ...
    for (i in seq_along(Rbv_names)) {
      Rbv_plots[[i]] <- df_Rbv %>%
        filter(metric == Rbv_names[i]) %>%
        ggplot(aes(x = Quantile, y = Error, group = Model, color = Model, linetype = Model)) +
        geom_line(size = 1) +
        scale_linetype_manual(values = linetypes) +
        scale_color_manual(values = color_scale) +
        scale_y_continuous(expand = c(0.02, 0)) +
        labs(title = NULL, x = if (i == 2) {
          "Probability level"
        } else {
          NULL
        }, y = Rbv_names[i], color = NULL, linetype = NULL) +
        theme(legend.position = legend.position) +
        coord_cartesian(ylim = Rbv_lims[[i]]) +
        if (factorize) {
          scale_x_discrete(expand = c(0.02, 0))
        } else {
          scale_x_continuous(expand = c(0.02, 0))
    ...
    Rbv_plot <- ggpubr::ggarrange(
      plotlist = Rbv_plots, nrow = 1, ncol = 3, labels = NULL,
...
plot_predictions_2D <- function(pred_eqrnn_test, pred_gbex_test, pred_grf_test, pred_exgam_test,
                                pred_semicond, ground_truth, X_test, prob_lvl_predicted,
                                legend.position = "bottom", show_title = FALSE) {
  title_q <- if (show_title) {
    paste0("Predictions (q=", prob_lvl_predicted, ")")
  } else {
    NULL
  }

  df <- data.frame(
    X1 = X_test[, 1], X2 = X_test[, 2], EQRNN = pred_eqrnn_test,
  ...
  pred2D <- df %>% ggplot(aes(X1, X2, z = Quantile)) +
    theme_minimal() +
    geom_raster(aes(fill = Quantile)) +
    scale_fill_continuous(name = latex2exp::TeX(paste0(r"(Quantile ($\tau=)", prob_lvl_predicted, "$):\t"))) +
    coord_fixed() +
    theme(
      aspect.ratio = 1, legend.position = legend.position, axis.ticks = element_blank(),
...
plot_validation_vs_resid <- function(results_tibble, groupby = "hidden_fct", add_title = "", loss = "Valid_loss", MSAE_prefix = "test_") {
  y_label <- if (loss == "Valid_loss") {
    "Validation loss"
  } else {
    str_replace_all(loss, "_", " ")
  }
  val_res_plt <- list()
...
plot_exceedences_quantile <- function(pred_eqrnn, y, quantile_levels, log_scaling = FALSE, factorize = TRUE, show_legend = FALSE) {
  n <- nrow(pred_eqrnn)
  nb_prob_lvls_predict <- length(quantile_levels)
  if (n != length(y)) {
    stop("Observation number mismatch between predictions and y in 'plot_exceedences_quantile'.")
  }
  if (nb_prob_lvls_predict != ncol(pred_eqrnn)) {
    stop("Quantile levels mismatch between predictions and quantile_levels in 'plot_exceedences_quantile'.")
  }
  ...
  trans_logp1 <- scales::trans_new(
    name = "logp1", transform = function(x) {
      log(x * 100 + 1)
    }, inverse = function(x) {
      (exp(x) - 1) / 100
    }, domain = c(-0, Inf),
  ...
  nex_plt <- df %>%
    tidyr::gather(key = "Exceedences", value = "Number", Expected, EQRNN, factor_key = TRUE) %>%
  ...
  if (log_scaling) {
    nex_plt <- nex_plt + scale_y_continuous(trans = trans_logp1, labels = logp1_ticks, expand = c(0.02, 0))
  } else {
    nex_plt <- nex_plt + scale_y_continuous(expand = c(0.02, 0))
  }
  if (!show_legend) {
    nex_plt <- nex_plt + theme(legend.position = "none")
  }
  return(nex_plt)
}
...
plot_rmse_threshold <- function(result_tbl, interm_lvls = "interm_lvl", predict_lvls = "Prob_lvl", errs = "test_MSE", err_sd = paste0(errs, "_sd"),
  ...
  if (factorize) {
    result_tbl[interm_lvls] <- as_factor(roundm(result_tbl[[interm_lvls]], 4))
  }
  ...
  rmse_plot <- result_tbl %>%
    ...
    geom_point(size = 2, color = my_palette_methods["EQRN"]) +
    geom_line(size = 1, color = my_palette_methods["EQRN"]) +
    scale_y_continuous(expand = c(0.02, 0)) +
    labs(title = NULL, x = "Intermediate probability level", y = y_lab_rmse) +
    facet_wrap(~ .data[[predict_lvls]], scales = "free_y", ncol = 3, strip.position = strip.position) # + theme(legend.position=legend.position)

  rmse_plot <- rmse_plot + if (factorize) {
    scale_x_discrete(expand = c(0.02, 0))
  } else {
    scale_x_continuous(expand = c(0.02, 0))
  }
  ...
  if (is.null(strip.position)) {
    rmse_plot <- rmse_plot + theme(strip.text.x = element_blank())
  }
  return(rmse_plot)
}
...
plot_rse_box_threshold <- function(residuals_tbl, interm_lvls = "interm_lvl", predict_lvls = "Prob_lvl", residuals = "test_resid",
                                   factorize = TRUE, test_data = "other", strip.position = "top", hide_ylab = FALSE) {
  if (factorize) {
    residuals_tbl[interm_lvls] <- as_factor(roundm(residuals_tbl[[interm_lvls]], 4))
  }

  # residuals_tbl$SEs <- residuals_tbl[[residuals]]^2
  residuals_tbl$AbsRes <- abs(residuals_tbl[[residuals]])

  rootmeansquared <- function(x, ...) {
    sqrt(mean(x^2, ...))
  }
  ...
  rse_plot <- rse_plot + if (factorize) {
    scale_x_discrete(expand = c(0.02, 0))
  } else {
    scale_x_continuous(expand = c(0.02, 0))
  }
  ...
  if (is.null(strip.position)) {
    rse_plot <- rse_plot + theme(strip.text.x = element_blank())
  }
  return(rse_plot)
}
...
plot_error_quantile <- function(fit_eqrn, fit_grf, fit_gbex, fit_egam, Y_train, X_test, prob_lvls_predict, intermediate_method = c("grf", "oracle"),
                                interm_lvl, interm_quant_train, model, distr, df, factorize = TRUE, test_data = "other") {
  ...
  df_mae <- data.frame(
    Quantile = Q_lvls, Uncond = MAEs_unc, Semi_cond = MAEs_semicond,
  ...
  mse_plot <- df_rmse %>%
    tidyr::gather(key = "Model", value = "Error", Uncond, Semi_cond, GRF, gpd_GAM, gbex, EQRNN, factor_key = TRUE) %>%
    ggplot(aes(x = Quantile, y = Error, group = Model, color = Model, linetype = Model)) +
    geom_line(size = 1) +
    scale_linetype_manual(values = linetypes) +
    scale_color_manual(values = color_scale) +
  ...
  mae_plot <- df_mae %>%
    ...
    scale_color_manual(values = color_scale) +
    scale_y_continuous(expand = c(0.02, 0)) +
    labs(title = NULL, x = "Probability level", y = "MAE", color = NULL, linetype = NULL)
  if (factorize) {
  ...
  err_plot <- ggpubr::ggarrange(mse_plot, mae_plot,
...
plot_error_distrib_results <- function(results_tibble, prob_lvl_predicted) {
  resid_box <- data.frame(
    Uncond_MSEs = results_tibble[[paste0("Uncond_test_MSE_q", prob_lvl_predicted)]],
    SemiCond_MSEs = results_tibble[[paste0("Semicond_test_MSE_q", prob_lvl_predicted)]],
    GRF_MSEs = results_tibble[[paste0("GRF_test_MSE_q", prob_lvl_predicted)]],
    EQRNN_MSEs = results_tibble[[paste0("EQRNN_test_MSE_q", prob_lvl_predicted)]],
    Uncond_MAEs = results_tibble[[paste0("Uncond_test_MAE_q", prob_lvl_predicted)]],
    SemiCond_MAEs = results_tibble[[paste0("Semicond_test_MAE_q", prob_lvl_predicted)]],
    GRF_MAEs = results_tibble[[paste0("GRF_test_MAE_q", prob_lvl_predicted)]],
    EQRNN_MAEs = results_tibble[[paste0("EQRNN_test_MAE_q", prob_lvl_predicted)]]
  ) %>%
    tidyr::gather(key = "Method", value = "Value", factor_key = TRUE) %>%
    ggplot(aes(x = Method, y = Value, group = Method, fill = Method)) +
    geom_abline(intercept = 0, slope = 0, col = "white", size = 1) +
    geom_violin(width = 1, color = "grey", alpha = 0.5) +
    stat_boxplot(geom = "errorbar", width = 0.5) +
    geom_boxplot(width = 0.75, color = "black", alpha = 0.8) +
    coord_flip() +
    stat_summary(fun = mean, geom = "point", shape = 4, size = 3, color = "black", fill = "black") +
    labs(title = paste0("Quantile Residuals (q=", prob_lvl_predicted, ")"), x = NULL, y = NULL) +
    theme(legend.position = "none", panel.grid.major.y = element_blank())
  return(resid_box)
}
