...
predict_gpd_gam <- function(fitted_gpd_gam, X_test, to_predict = c(0.95, 0.99),
  ...
  if (interm_lvl != fitted_gpd_gam$interm_lvl) {
    stop("gpd_gam intermediate quantiles interm_lvl does not match in train and predict.")
  }

  X_feats <- X_test
  if (fitted_gpd_gam$intermediate_q_feature) {
    if (is.null(intermediate_quantiles)) {
      stop("intermediate_quantiles needed in predict_gpd_gam for intermediate_q_feature.")
    }
...
excess_probability_gpd_gam <- function(fitted_gpd_gam, val, X_test, intermediate_quantiles, interm_lvl = fitted_gpd_gam$interm_lvl,
  ...
  Probs <- GPD_excess_probability(val,
    sigma = pred_params$sigma, xi = pred_params$xi, interm_threshold = intermediate_quantiles,
